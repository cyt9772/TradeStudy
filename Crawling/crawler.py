...
class Crawler:
    ...
    def get_news_section(self):
        soup = BeautifulSoup(self.html_doc, 'html.parser') #html_doc parsing with html.parser
        print(soup.prettify()) #check if the html was parsed normally, making look good the html
        print("title", soup.title)
        #<title>Home</title>
        print("title stinrg", soup.title.string) #get only for time-> "Home"
        ...
        """
        print("find href", soup.find_all(href=re.compile("/news"))) #/news를 포함하는 모든 href 찾기
    ...
    def get_side(self):
        ...
        col_list = [ col.string for col in soup.thead.find_all(scope=re.compile("col"))] #only for value
        print(col_list)
        #['컬럼1', '컬럼2']
