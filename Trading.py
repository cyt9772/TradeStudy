...
from Scheduler.mongodb_handler import MongoDBHandler
...
def check_buy_order(code):
    """매수주문 완료 체크"""
    order_list=list(mongo.find_items({"$and":[{"code":code},{"status":"buy_ordered"}]}, "stocklab_demo","order"))
    for order in order_list:
        time.sleep(1)
        code=order["code"]
        order_no=order["매수번호"]["주문번호"]
...
def check_sell_order(code):
    """ 매도주문 완료 체크"""
    sell_order_list = list(mongo.find_items({"$and": [{"code": code}, {"status": "sell_ordered"}]}, "stocklab_demo", "order"))
    for order in sell_order_list:
        time.sleep(1)
        code = order["code"]
        order_no = order["매도번호"]["주문번호"]
        order_cnt = order["매도주문"]["실물주문수량"]
        check_result = ebest_demo.order_check(order_no)
        print("check sell order result", check_result)
        result_cnt = check_result["체결수량"]
        if order_cnt == result_cnt:
            mongo.update_item({"매도주문.주문번호": order_no}, {"$set": {"매도완료": check_result, "status": "sell_completed"}},
                              "stocklab_demo", "sell_order")
...
def trading_scenario(code_list):
    for code in code_list:
        ...
        if buy_order_cnt==0:
            ...
            order_doc=order[0]
            mongo.insert_item({"매수주문":order_doc, "code":code, "satus":"buy_ordered"},"stocklab_demo","order")
...
if __name__=="__main__":
    ...
    scheduler.add_job(func=run_process_trading_scenario,trigger="interval", minutes=0.2,id="demo",kwargs={"code_list":code_list})
    scheduler.start()
